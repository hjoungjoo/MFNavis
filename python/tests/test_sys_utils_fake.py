...
class TestRestoreUserdata:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(fake_backup_path, "w") as zipf:
                    zipf.write(
                        fake_config_path, "home/pifinder/PiFinder_data/config.json"
                    )
    ...
    def test_restore_fails_with_different_observations_content(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(fake_backup_path, "w") as zipf:
                    zipf.write(
                        fake_obs_path, "home/pifinder/PiFinder_data/observations.db"
                    )
    ...
    def test_restore_fails_with_invalid_zip_structure(self):
        ...
        try:
            ...
            assert "Invalid backup file: missing expected directory structure" in str(
                exc_info.value
            )
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(fake_backup_path, "w") as zipf:
                    zipf.write(
                        fake_file_path,
                        "home/pifinder/PiFinder_data/nonexistent_file.txt",
                    )
