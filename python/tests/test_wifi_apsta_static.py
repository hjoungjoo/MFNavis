...
def test_apsta_system_files_target_virtual_ap_interface():
    ...
    prepare_service = (
        REPO / "pi_config_files/mfnavis_apsta_prepare.service"
    ).read_text()
    monitor_service = (
        REPO / "pi_config_files/mfnavis_apsta_monitor.service"
    ).read_text()
    ...
    assert (
        "ExecStart=/usr/bin/bash __MFNAVIS_REPO_DIR__/scripts/mfnavis_apsta.sh prepare"
        in prepare_service
    )
    assert (
        "ExecStart=/usr/bin/bash __MFNAVIS_REPO_DIR__/scripts/mfnavis_apsta.sh monitor"
        in monitor_service
    )
    assert 'AP_IFACE="${MFNAVIS_AP_IFACE:-uap0}"' in manager
    ...
    assert "MFNAVIS_APSTA_SHARE_INTERNET=1" in manager
...
def test_wifi_profile_import_is_installed_by_setup_and_update():
    setup = (REPO / "mfnavis_setup.sh").read_text()
    post_update = (REPO / "mfnavis_post_update.sh").read_text()
    migration = (REPO / "migration_source/mf_wifi_settings.sh").read_text()
    paths = (REPO / "mfnavis_paths.sh").read_text()
    ...
    assert "mfnavis_prepare_wpa_supplicant_config" in paths
    assert "mfnavis_prepare_apsta_nat_config" in paths
    assert "mfnavis_prepare_sta_band_config" in paths
    assert "MFNAVIS_APSTA_SHARE_INTERNET=0" in paths
    assert "MFNAVIS_STA_BAND=auto" in paths
    assert "chmod 600 /etc/wpa_supplicant/wpa_supplicant.conf" in paths


def test_wifi_credentials_are_not_made_world_readable():
    for path in [
        REPO / "mfnavis_setup.sh",
        REPO / "mfnavis_paths.sh",
        REPO / "mfnavis_post_update.sh",
