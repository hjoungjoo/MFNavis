# MFDS 공개 저장소 전환 기록 — 2026-09-16

> 아래는 과거 소스 서브모듈 이관 기록이다. 현재 설치는 [MFDS 바이너리 배포 안내](MFDS_BINARY_DISTRIBUTION_ko.md)를 따른다.
...
소스 연결 경로를 python/MFDS로 통일한 후, MFDS v0.3.0부터 소비 방식을 빌드된 릴리즈 패키지로 전환했다.
현재 PiFinder에는 MFDS 소스 서브모듈이 없으며 다운로드된 패키지를 편집하지 않는다.
[현재 설치 및 관리 절차](MFDS_BINARY_DISTRIBUTION_ko.md)를 참고한다.
